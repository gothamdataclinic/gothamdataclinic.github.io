...
**A quirk worth knowing — origin detection behind a reverse proxy:** anywhere the server needs to build an absolute URL (the OAuth callback's post-login redirect, the root-path proxy), it must not use `new URL(request.url).origin` directly. Next.js under `next start` builds that from the raw `Host` header it receives, and the hosting platform's proxy (Vercel's edge network today; Render's, previously) forwards requests to the actual container/function using an internal host rather than the public hostname — so naive origin detection can resolve to an internal address in production instead of `cms.gothamdataclinic.org`. `lib/getRequestOrigin.ts` fixes this by trusting `X-Forwarded-Host`/`X-Forwarded-Proto` (which both platforms set) and only falling back to `request.url`'s origin for local dev where those headers aren't present.
...
**Database:** Supabase Postgres via the session pooler or direct connection (port 5432) — not the transaction pooler (port 6543), which breaks Payload's need for prepared statements. The session pooler's client-connection cap (Supavisor's own `pool_size`, distinct from and much smaller than Postgres's own `max_connections`) is a real constraint: Vercel keeps serverless instances warm between requests, and `node-postgres`'s default of up to 10 connections per instance was enough for a handful of concurrent instances to exhaust a `pool_size: 15` cap entirely — locking out the admin panel, the public API, and even a direct `psql` connection until an instance happened to cool down. Fixed on two sides: `postgresAdapter`'s pool is explicitly capped at `max: 3` per instance (`payload.config.ts`), and the pooler's `pool_size` was raised to 40 in the Supabase dashboard (Settings → Database → Connection Pooling). **Dev-mode schema auto-push is disabled** (`push: false` in the `postgresAdapter` config) — with it on, removing or renaming a field/global makes Payload block every request behind an interactive "drop this table? (y/N)" prompt on boot, which just hangs in a non-interactive process (discovered when splitting `SiteSettings` into per-page globals below). Schema changes go through an explicit `payload migrate:create` / `payload migrate` now (`cms/src/migrations/`) — two tracked migrations exist: an initial baseline, and one covering the `SiteSettings` → per-page-globals split plus the `events.website_url` column and `Interview` publication-type enum value that had drifted in via dev-push before `push: false` was set. The second one was hand-authored rather than generated, since `migrate:create`'s interactive rename-detection prompt (drizzle-kit) has no non-interactive mode — it was written by introspecting the live (already-correct) schema directly and verified idempotent before running against production.
...
- **`cms/`** → Vercel, root directory `cms`, deployed automatically on push to `main` via Vercel's GitHub App integration (installed on the `gothamdataclinic` org). Served at `cms.gothamdataclinic.org` (a custom domain on top of Vercel's own `*.vercel.app` deployment URL). No Vercel config is checked into this repo (no `vercel.json`) — env vars are set directly in the Vercel dashboard (Project Settings → Environment Variables), including `GITHUB_TOKEN`, which the CMS's own `afterChange` hooks (`cms/src/lib/triggerRebuild.ts`) use to fire a `repository_dispatch` back at this repo on every content save — that's what triggers the website rebuild below without a manual `git push`. (The CMS previously ran on Render, then briefly Railway, before settling on Vercel in early August 2026 — both of the prior platforms' GitHub App installations have since been removed from the org.)
